...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate LSTM image captioning model on RSICD")
    parser.add_argument(
        "--model_path",
        type=str,
...
def build_model(config: Dict, vocab_len: int, device: str) -> ImageCaptioningModel:
    ...
    decoder = Decoder(embed_dim=projected_dim, hidden_dim=hidden_dim, vocab_size=vocab_len)
...
def main() -> None:
    ...
    output_path = os.path.join(
        args.results_dir,
        f"evaluation_results_lstm_{checkpoint_stem}_{args.decode}.json",
    )
